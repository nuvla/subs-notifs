...
PROCESSING_TIME = Gauge(
    ...
    'Time spent in processing of a telemetry packet or event',
    ['type', 'message_info'], namespace=namespace)

PACKETS_PROCESSED = Counter(
    'packets_processed',
    'Number of telemetry packets processed',
    ['type', 'id'], namespace=namespace)
PACKETS_ERROR = Counter(
    'packets_error',
    'Number of telemetry packets processed with error',
    ['type', 'id', 'exception'], namespace=namespace)

SUBSCRIPTION_CONFIGS = Gauge(
    'subscription_configs',
    'Number of subscription configurations',
...
ES_INDEX_DOCS_DELETED = Counter(
    'es_index_docs_deleted',
    'Number of documents deleted from an index',
    ['index', 'id'], namespace=namespace)

INTERVENTION_ERRORS = Gauge(
    'intervention_errors',
    'Error occurred in intervention',
